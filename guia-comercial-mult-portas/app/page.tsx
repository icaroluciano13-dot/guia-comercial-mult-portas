...
const STORAGE = {
  ...
  catalogChecks: "mult-portas-guia-catalog-checks-v1",
};
...
const AUTH_PROFILES_KEY = "mult-portas-guia-profiles-v1";
const AUTH_SESSION_KEY = "mult-portas-guia-session-v1";
...
function scopedStorage(profileId: string) {
  return Object.fromEntries(
    Object.entries(STORAGE).map(([key, value]) => [key, `${value}:${profileId}`]),
...
function safeStorageSet(key: string, value: string) {
  try {
    ...
    // O painel continua funcionando mesmo quando o navegador bloqueia o armazenamento local.
  }
}
...
function readProfiles(): GuideProfile[] {
  try {
    const raw = JSON.parse(localStorage.getItem(AUTH_PROFILES_KEY) || "[]");
    if (!Array.isArray(raw)) return [];
    return raw.filter((profile): profile is GuideProfile => Boolean(
      profile && typeof profile === "object" &&
      typeof profile.id === "string" &&
      typeof profile.username === "string" &&
      typeof profile.displayName === "string" &&
      typeof profile.passwordHash === "string" &&
      (profile.branch === "Araraquara" || profile.branch === "São Carlos"),
    ));
  } catch {
    return [];
  }
...
function hexToBytes(hex: string) {
  if (!/^[0-9a-f]+$/i.test(hex) || hex.length % 2 !== 0) throw new Error("Salt inválido.");
  return new Uint8Array(hex.match(/.{1,2}/g)?.map((pair) => Number.parseInt(pair, 16)) ?? []);
}

async function hashGuidePassword(password: string, storedSalt?: string) {
  if (!globalThis.crypto?.subtle) throw new Error("Criptografia não disponível neste navegador.");
  const salt = storedSalt ? hexToBytes(storedSalt) : globalThis.crypto.getRandomValues(new Uint8Array(16));
  const data = new TextEncoder().encode(password);
  const key = await globalThis.crypto.subtle.importKey("raw", data, "PBKDF2", false, ["deriveBits"]);
...
async function verifyGuidePassword(password: string, storedHash: string) {
  const [salt, digest] = storedHash.split(":");
  if (!salt || !digest) return false;
  return await hashGuidePassword(password, salt) === storedHash;
}

function migrateLegacyWorkspace(profileId: string) {
  const target = scopedStorage(profileId);
  for (const key of Object.keys(STORAGE) as (keyof typeof STORAGE)[]) {
    if (localStorage.getItem(target[key])) continue;
    const legacyValue = localStorage.getItem(STORAGE[key]);
    if (legacyValue) safeStorageSet(target[key], legacyValue);
  }
}
...
function GuideAuth({ mode, form, error, busy, onModeChange, onSubmit, setForm }: GuideAuthProps) {
    ...
    <main className="auth-shell">
      <section className="auth-card" aria-labelledby="auth-title">
        <div className="auth-brand">
          <div className="brand-mark auth-brand-mark">MP</div>
          <div><strong>GUIA MULT PORTAS</strong><small>Acesso interno para funcionários</small></div>
        </div>
        <div className="auth-heading">
          <span className="section-kicker"><span className="eyebrow-line" /> PERFIL INDIVIDUAL</span>
          <h1 id="auth-title">{isRegister ? "Crie seu perfil de trabalho." : "Entre no seu painel."}</h1>
          <p>{isRegister ? "Cadastre usuário e senha para manter seu espaço comercial separado." : "Entre com seu usuário e sua senha para acessar seu painel comercial e seus registros individuais."}</p>
        </div>

        <div className="auth-tabs" role="tablist" aria-label="Acesso ao Guia">
          <button type="button" className={mode === "login" ? "active" : ""} onClick={() => onModeChange("login")} role="tab" aria-selected={mode === "login"}>Entrar</button>
          <button type="button" className={mode === "register" ? "active" : ""} onClick={() => onModeChange("register")} role="tab" aria-selected={mode === "register"}>Cadastrar perfil</button>
        </div>

        <form className={`auth-form ${isRegister ? "register" : ""}`} onSubmit={onSubmit}>
          <div className="auth-profile-preview">
            <div className="auth-avatar">MP</div>
            <div><strong>{isRegister ? "Novo funcionário" : "Acesso do funcionário"}</strong><small>Dados comerciais separados por perfil</small></div>
          </div>

          <label><span>Usuário de login</span><input required value={form.username} onChange={(event) => update("username", event.target.value)} placeholder="Ex.: vendedor01" autoComplete={isRegister ? "username" : "current-username"} minLength={3} maxLength={32} /><small>Use letras, números, ponto, hífen ou underline.</small></label>

          {isRegister && <>
            <div className="auth-grid">
              <label><span>Nome completo</span><input required value={form.displayName} onChange={(event) => update("displayName", event.target.value)} placeholder="Ex.: Nome do funcionário" autoComplete="name" maxLength={80} /></label>
              <label><span>Função</span><input value={form.jobTitle} onChange={(event) => update("jobTitle", event.target.value)} placeholder="Ex.: Consultor comercial" autoComplete="organization-title" maxLength={80} /></label>
              <label><span>Telefone</span><input value={form.phone} onChange={(event) => update("phone", event.target.value)} placeholder="(16) 99999-9999" autoComplete="tel" maxLength={24} /></label>
              <label><span>Filial</span><select required value={form.branch} onChange={(event) => update("branch", event.target.value as GuideProfile["branch"])}><option value="Araraquara">Araraquara</option><option value="São Carlos">São Carlos</option></select></label>
            </div>
            <label><span>Cidade / região</span><input value={form.cityRegion} onChange={(event) => update("cityRegion", event.target.value)} placeholder="Ex.: Araraquara e região" maxLength={100} /></label>
          </>}

          <div className={isRegister ? "auth-grid" : ""}>
            <label><span>Senha do Guia {isRegister && "(obrigatória)"}</span><input required type="password" value={form.password} onChange={(event) => update("password", event.target.value)} placeholder="Mínimo de 6 caracteres" autoComplete={isRegister ? "new-password" : "current-password"} minLength={6} maxLength={128} /></label>
            {isRegister && <label><span>Confirmar senha (obrigatória)</span><input required type="password" value={form.passwordConfirm} onChange={(event) => update("passwordConfirm", event.target.value)} placeholder="Repita a senha do Guia" autoComplete="new-password" minLength={6} maxLength={128} /></label>}
          </div>

          {error && <p className="auth-error" role="alert">{error}</p>}
          <p className="auth-note">✓ A senha é exclusiva do Guia Comercial. Seus dados permanecem salvos quando você sair.</p>
          <button className="button dark auth-submit" type="submit" disabled={busy}>{busy ? "Aguarde…" : isRegister ? "Criar meu perfil" : "Entrar no Guia"}<span>→</span></button>
        </form>
      </section>
    </main>
  );
...
export default function Home() {
  ...
  const [activeProfile, setActiveProfile] = useState<GuideProfile | null>(null);
  ...
  const profileStorage = useMemo(
    () => scopedStorage(activeProfile?.id ?? "guest"),
    [activeProfile?.id],
  );
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      try {
        ...
        if (sessionId) setActiveProfile(savedProfiles.find((profile) => profile.id === sessionId) ?? null);
      } catch {
  ...
  useEffect(() => {
    if (!authReady || !activeProfile) return;
    ...
    const timer = window.setTimeout(() => {
      ...
      const savedSales = readLocalJson(profileStorage.sales, []);
      const savedTiming = readLocalJson(profileStorage.timing, []);
      const savedFollowUps = readLocalJson<LocalFollowUp[] | null>(profileStorage.followups, null);
      const savedChecks = readLocalJson(profileStorage.checks, []);
      const savedMetrics = readLocalJson<Record<string, unknown> | null>(profileStorage.metrics, null);
      const savedTraining = readLocalJson<Record<string, unknown> | null>(profileStorage.training, null);
      const savedMessages = readLocalJson<Record<string, unknown> | null>(profileStorage.messages, null);
      const savedFactory = readLocalJson<unknown>(profileStorage.factory, null);
      const savedCatalogChecks = readLocalJson<Record<string, string[]>>(profileStorage.catalogChecks, {});
        ...
        if (savedCatalogChecks && typeof savedCatalogChecks === "object" && !Array.isArray(savedCatalogChecks)) setDrawerChecks(savedCatalogChecks);
    ...
    return () => window.clearTimeout(timer);
  }, [activeProfile, authReady, profileStorage]);
  ...
  useEffect(() => {
    if (!authReady || !activeProfile || !hydrated) return;
    safeStorageSet(profileStorage.sales, JSON.stringify(doneSales));
    safeStorageSet(profileStorage.timing, JSON.stringify(doneTiming));
    safeStorageSet(profileStorage.followups, JSON.stringify(followUps));
    safeStorageSet(profileStorage.checks, JSON.stringify(dailyDone));
    safeStorageSet(profileStorage.metrics, JSON.stringify(metrics));
    safeStorageSet(profileStorage.training, JSON.stringify(trainingStats));
    safeStorageSet(profileStorage.factory, JSON.stringify(factoryItems.map((item) => ({ ...item, manufacturer: "DALCOMAD" }))));
    safeStorageSet(profileStorage.catalogChecks, JSON.stringify(drawerChecks));
    safeStorageSet(profileStorage.messages, JSON.stringify({
      name: messageName,
      line: messageLine,
  ...
  }, [activeProfile, authReady, dailyDone, doneSales, doneTiming, drawerChecks, factoryItems, followUps, hydrated, messageChannel, messageEnvironment, messageLine, messageName, messageObjective, messageProof, messageQuestion, messageTone, metrics, profileStorage, trainingStats]);
  ...
  function changeAuthMode(nextMode: AuthMode) {
    ...
    setAuthError("");
    setAuthForm((current) => ({ ...EMPTY_AUTH_FORM, username: current.username }));
  }
  ...
  async function submitAuth(event: FormEvent<HTMLFormElement>) {
    ...
    if (!/^[a-z0-9._-]{3,32}$/.test(username)) {
      setAuthError("Use um usuário com 3 a 32 caracteres, sem espaços ou acentos.");
      return;
    }
    ...
    try {
      if (authMode === "register") {
        if (!authForm.displayName.trim()) {
          setAuthError("Informe o nome completo do funcionário.");
          return;
        }
        ...
        if (profiles.some((profile) => normalizeUsername(profile.username) === username)) {
          setAuthError("Esse usuário já está cadastrado. Escolha outro.");
          return;
        }

        const profile: GuideProfile = {
          id: globalThis.crypto?.randomUUID?.() || `profile-${username}-${profiles.length + 1}`,
          username,
          displayName: authForm.displayName.trim(),
          jobTitle: authForm.jobTitle.trim() || "Consultor comercial",
          phone: authForm.phone.trim(),
          branch: authForm.branch,
          cityRegion: authForm.cityRegion.trim(),
          passwordHash: await hashGuidePassword(authForm.password),
          createdAt: new Date().toISOString(),
        };
        const nextProfiles = [...profiles, profile];
        if (profiles.length === 0) migrateLegacyWorkspace(profile.id);
        localStorage.setItem(AUTH_PROFILES_KEY, JSON.stringify(nextProfiles));
        localStorage.setItem(AUTH_SESSION_KEY, profile.id);
      ...
      const profile = profiles.find((candidate) => normalizeUsername(candidate.username) === username);
      if (!profile || !await verifyGuidePassword(authForm.password, profile.passwordHash)) {
      ...
      setAuthError("Não foi possível salvar o perfil neste navegador. Verifique o armazenamento e tente novamente.");
    } finally {
      setAuthBusy(false);
    }
  }
  ...
  function openProfileEditor() {
    ...
    setProfileEditForm({
      ...
      cityRegion: activeProfile.cityRegion,
      newPassword: "",
      passwordConfirm: "",
    });
    setProfileError("");
  ...
  async function saveProfile(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!activeProfile || !profileEditForm || profileBusy) return;
    setProfileError("");
    if (!profileEditForm.displayName.trim()) {
      setProfileError("Informe o nome completo do funcionário.");
      return;
    }
    if (profileEditForm.newPassword && profileEditForm.newPassword.length < 6) {
      setProfileError("A nova senha precisa ter pelo menos 6 caracteres.");
      return;
    }
    if (profileEditForm.newPassword !== profileEditForm.passwordConfirm) {
      setProfileError("As senhas de alteração não coincidem.");
      return;
    }
    ...
    try {
      const updatedProfile: GuideProfile = {
        ...
        jobTitle: profileEditForm.jobTitle.trim() || "Consultor comercial",
        phone: profileEditForm.phone.trim(),
        branch: profileEditForm.branch,
        cityRegion: profileEditForm.cityRegion.trim(),
        passwordHash: profileEditForm.newPassword
          ? await hashGuidePassword(profileEditForm.newPassword)
          : activeProfile.passwordHash,
      };
      ...
      setProfileError("Não foi possível salvar o perfil neste navegador.");
    } finally {
  ...
  function logoutGuide() {
    trainingRequestRef.current += 1;
    resetTraining();
    try {
      localStorage.removeItem(AUTH_SESSION_KEY);
    } catch {
      // A sessão visual ainda é encerrada quando o navegador bloqueia o armazenamento.
    }
    setActiveProfile(null);
    setHydrated(false);
    setAuthMode("login");
  ...
  async function sendTrainingMessage(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (requestId === trainingRequestRef.current) setTrainingBusy(false);
    }
  }
  ...
  function addFollowUp(event: FormEvent<HTMLFormElement>) {
    ...
    setFollowUps((current) => [{ id: globalThis.crypto?.randomUUID?.() || `local-${current.length + 1}-${newClient.trim().toLowerCase().replace(/[^a-z0-9]+/g, "-")}`, client: newClient.trim(), status: newStatus, next: newNext.trim(), priority: newPriority, done: false }, ...current]);
    setNewClient("");
    setNewNext("");
  ...
  function addFactoryWizardItem() {
    ...
    const generatedItem: FactoryRequestItem = {
      id: globalThis.crypto?.randomUUID?.() || `wizard-${factoryItems.length + 1}`,
      manufacturer: "DALCOMAD",
      ...factoryWizardDraft,
      priceWithoutLock: "",
      priceWithLock: "",
  ...
  if (!authReady) {
    return <main className="auth-shell auth-loading-shell" aria-live="polite"><div className="auth-loading-card">Preparando o acesso ao Guia…</div></main>;
  }

  if (!activeProfile) {
    return <GuideAuth mode={authMode} form={authForm} error={authError} busy={authBusy} onModeChange={changeAuthMode} onSubmit={submitAuth} setForm={setAuthForm} />;
  }

  if (!hydrated) {
    return <main className="auth-shell auth-loading-shell" aria-live="polite"><div className="auth-loading-card">Carregando o painel de {activeProfile.branch}…</div></main>;
  }

  const profileBadge = activeProfile.displayName
    .split(/\s+/)
    .filter(Boolean)
    .slice(0, 2)
    .map((part) => part[0])
    ...
    <main className="app-shell">
      ...
      <section className="content-shell">
        <header className="topbar">
          ...
          <div className="topbar-actions"><span className="date-chip">{today}</span><span className="profile-name">{activeProfile.displayName}</span><button className="user-chip" type="button" onClick={openProfileEditor} title="Editar meu perfil" aria-label="Editar meu perfil">{profileBadge}</button><button className="logout-button" type="button" onClick={logoutGuide}>Sair do Guia</button></div>
        </header>
            ...
            <div className="message-layout">
              <section className="panel message-form-panel">
                ...
                <div className="message-form-grid">
                  <label><span>Nome do cliente (opcional)</span><input value={messageName} onChange={(event) => setMessageName(event.target.value)} placeholder="Ex.: Nome do cliente" /></label>
      ...
      {profileDrawerOpen && activeProfile && profileEditForm && (
        <div className="profile-drawer-backdrop" onClick={() => setProfileDrawerOpen(false)}>
          <section className="profile-dialog" role="dialog" aria-modal="true" aria-labelledby="profile-dialog-title" onClick={(event) => event.stopPropagation()}>
            <button className="profile-dialog-close" type="button" aria-label="Fechar edição do perfil" onClick={() => setProfileDrawerOpen(false)}>×</button>
            <span className="section-kicker">MEU PERFIL</span>
            <h2 id="profile-dialog-title">Editar perfil</h2>
            <p className="profile-dialog-copy">Atualize seus dados de funcionário e mantenha seu espaço de trabalho organizado.</p>
            <form className="profile-edit-form" onSubmit={saveProfile}>
              <div className="profile-edit-grid">
                <label><span>Nome completo</span><input required value={profileEditForm.displayName} onChange={(event) => setProfileEditForm((current) => current ? { ...current, displayName: event.target.value } : current)} maxLength={80} autoComplete="name" /></label>
                <label><span>Função</span><input value={profileEditForm.jobTitle} onChange={(event) => setProfileEditForm((current) => current ? { ...current, jobTitle: event.target.value } : current)} maxLength={80} autoComplete="organization-title" /></label>
                <label><span>Telefone</span><input value={profileEditForm.phone} onChange={(event) => setProfileEditForm((current) => current ? { ...current, phone: event.target.value } : current)} maxLength={24} autoComplete="tel" /></label>
                <label><span>Filial</span><select value={profileEditForm.branch} onChange={(event) => setProfileEditForm((current) => current ? { ...current, branch: event.target.value as GuideProfile["branch"] } : current)}><option value="Araraquara">Araraquara</option><option value="São Carlos">São Carlos</option></select></label>
              </div>
              <label><span>Cidade / região</span><input value={profileEditForm.cityRegion} onChange={(event) => setProfileEditForm((current) => current ? { ...current, cityRegion: event.target.value } : current)} maxLength={100} placeholder="Ex.: Araraquara e região" /></label>
              <div className="profile-password-heading"><span className="section-kicker">ALTERAR SENHA</span><small>Opcional — deixe em branco para manter a senha atual.</small></div>
              <div className="profile-edit-grid">
                <label><span>Nova senha</span><input type="password" value={profileEditForm.newPassword} onChange={(event) => setProfileEditForm((current) => current ? { ...current, newPassword: event.target.value } : current)} minLength={6} maxLength={128} autoComplete="new-password" placeholder="Mínimo de 6 caracteres" /></label>
                <label><span>Confirmar nova senha</span><input type="password" value={profileEditForm.passwordConfirm} onChange={(event) => setProfileEditForm((current) => current ? { ...current, passwordConfirm: event.target.value } : current)} minLength={6} maxLength={128} autoComplete="new-password" placeholder="Repita a nova senha" /></label>
              </div>
              {profileError && <p className="auth-error" role="alert">{profileError}</p>}
              <div className="profile-dialog-actions"><button className="button light" type="button" onClick={() => setProfileDrawerOpen(false)}>Cancelar</button><button className="button dark" type="submit" disabled={profileBusy}>{profileBusy ? "Salvando…" : "Salvar alterações"}<span>✓</span></button></div>
            </form>
          </section>
        </div>
      )}
